...
static void retransmit_handler(void *arg)
{
	...
	reply->txc++;
	tmr_start(&reply->tmrg,
		  MIN(sip_conf(reply->sess->sip)->sip_t1<<reply->txc,
		      sip_conf(reply->sess->sip)->sip_t2),
		      ...
		      const char *fmt, va_list *ap)
{
	...
	tmr_start(&reply->tmr, 64 * sip_conf(sess->sip)->sip_t1,
		  tmr_handler, reply);
	tmr_start(&reply->tmrg, sip_conf(sess->sip)->sip_t1,
		  retransmit_handler, reply);
