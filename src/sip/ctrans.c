...
static void retransmit_handler(void *arg)
{
	...
	case TRYING:
		timeout = MIN(sip_conf(ct->sip)->sip_t1<<ct->txc,
			      sip_conf(ct->sip)->sip_t2);
		break;

	case CALLING:
		timeout = sip_conf(ct->sip)->sip_t1<<ct->txc;
		break;

	case PROCEEDING:
		timeout = sip_conf(ct->sip)->sip_t2;
		break;
...
static bool response_handler(const struct sip_msg *msg, void *arg)
{
	...
	case PROCEEDING:
		if (msg->scode < 200) {
			...
			tmr_start(&ct->tmr, sip_conf(ct->sip)->sip_t4,
				  tmr_handler, ct);
			tmr_cancel(&ct->tmre);
		}
		break;
		       ...
		       sip_resp_h *resph, void *arg)
{
	...
	tmr_start(&ct->tmr, 64 * sip_conf(sip)->sip_t1, tmr_handler, ct);

	if (!sip_transp_reliable(ct->tp)) {
		tmr_start(&ct->tmre, sip_conf(sip)->sip_t1,
			  retransmit_handler, ct);
	}
...
int sip_ctrans_cancel(struct sip_ctrans *ct)
{
	...
	case PROCEEDING:
		tmr_start(&ct->tmr, 64 * sip_conf(ct->sip)->sip_t1,
			  tmr_handler, ct);
		break;
