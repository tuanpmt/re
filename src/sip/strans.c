...
static void retransmit_handler(void *arg)
{
	...
	st->txc++;
	tmr_start(&st->tmrg, MIN(sip_conf(st->sip)->sip_t1<<st->txc,
				 sip_conf(st->sip)->sip_t2),
...
static bool ack_handler(struct sip *sip, const struct sip_msg *msg)
{
	...
	case COMPLETED:
		...
		tmr_start(&st->tmr, sip_conf(sip)->sip_t4, tmr_handler, st);
		tmr_cancel(&st->tmrg);
		st->state = CONFIRMED;
		break;
		     ...
		     uint16_t scode, struct mbuf *mb)
{
	...
	if (st->invite) {
		if (scode < 200) {
			...
			tmr_start(&st->tmr, 64 * sip_conf(sip)->sip_t1,
				  tmr_handler, st);
			st->state = ACCEPTED;
		}
		else {
			...
			if (!sip_transp_reliable(st->msg->tp))
				tmr_start(&st->tmrg, sip_conf(sip)->sip_t1,
					  retransmit_handler, st);
		}
	}
	else {
